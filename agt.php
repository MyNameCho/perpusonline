...
if (isset($_POST['daftar'])) {
    ...
    // Query untuk memeriksa apakah nama anggota sudah ada
    $sql = $koneksi->query("SELECT * FROM tb_anggota WHERE nama='$nama'");
    if ($sql->num_rows > 0) {
        echo "<script>alert('Nama sudah digunakan!');</script>";
    } else {
        // Menambahkan level 'Anggota' dan informasi tambahan lainnya saat registrasi
        $sql = $koneksi->query("INSERT INTO tb_anggota (nama, jekel, kelas, no_hp) 
                                 VALUES ('$nama', '$jekel', '$kelas', '$alamat')");
        if ($sql) {
...
<body>
    <div class="container">
        ...
        <div class="link">
            <p>Sudah punya member? <a href="login.php">Login di sini</a></p>
        </div>
    </div>
</body>
</html>
